	...
	var nTotalSize = 0;
	var nMaxImageSize = 25*1024*1024;
		...
		if(typeof ev.dataTransfer.files == 'undefined'){
			...
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type)) {
					...
					alert("이미지 용량이 25MB를 초과하여 등록할 수 없습니다.");
				} else {
					//제한된 수만 업로드 가능.
					if ( j < nMaxImageCount ){
    ...
    function html5Upload() {	
    	...
    	sUploadURL = '/StudyProject/file_uploader_html5.jsp' //upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 	    	...
 	    	sUrl : '/StudyProject/file_uploader.jsp',
 	    	sCallback : '/StudyProject/SE2/photo_uploader/popup/callback.html',
