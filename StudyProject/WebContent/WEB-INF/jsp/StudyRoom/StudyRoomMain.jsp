...
/* 	
	...
	function sendIt()
	{
		...
		{
			f.action = "StudyRoomSearch.room?searchKey="+searchKey+"&searchValue="+searchValue;	
		}
...
<div class="row">
	...
	<div class="form-group col-sm-8" >
		...
		<table class="table">
				<tr>
					<th style="text-align: center;" >순　위</th>
			...
			<c:forEach var="SreBoard1" items="${studyApplyList1}">
				<tr style="text-align: center;">
					...
					<td style="text-align: center;">${fn:substring(SreBoard1.hnwDate,0,10)}</td>
			...
			<table id="StudyRoomMain" class="table" >
				...
				<c:forEach var="SreBoard" items="${studyApplyList}">
					<tr style="text-align: center;">
						...
						<td style="text-align: center;">${fn:substring(SreBoard.hnwDate,0,10)}</td>
						<%-- 
						<td>${SreBoard.souNd }</td>
						<td>${SreBoard.yesNo }</td>
						<td>${SreBoard.wiFi }</td>
					...
					<form action="" id="searchForm" name="searchForm" method="post">
						<div class="col-xs-4">
							<select name="searchKey"  id="searchKey" class="form-control" >
								...
								<option value="CONSENT">콘센트</option>
								<option value="MINTIME">WIFI</option>
								<option value="YESNO">냉난방기</option>
							</select>
						</div>
						<div class="col-xs-7">
							<input type="text" class="form-control" id="searchValue" placeholder="검색어 입력">							
						</div>
						<div class="col-xs-2">
						<button type="button" class="btn btn-primary cs-btn" onclick="sendIt()">검색</button>
						</div>	
					</form>
				</div>
		</div>
