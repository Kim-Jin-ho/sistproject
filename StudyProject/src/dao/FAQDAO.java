...
public class FAQDAO implements IFAQDAO
{
	...
	public ArrayList<FAQDTO> list() throws SQLException
	{
		...
		String sql = "SELECT FAQID, SRAID, FAQNAME, FAQDAP FROM SRFAQ ORDER BY 1 DESC";
		System.out.println("아래의 쿼리문을 실행합니다.\n" + sql);
		...
		while(rs.next())
		{
			...
			dto.setFaqid(rs.getString("FAQID"));
			dto.setSraid(rs.getString("SRAID"));
			dto.setFaqname(rs.getString("FAQNAME"));
			dto.setFaqdap(rs.getString("FAQDAP"));
...
/*
	...
	public int getDataCount() throws SQLException
	{
		...
		String sql = "SELECT COUNT(FAQID) AS COUNT FROM SRFAQ";
		System.out.println("아래의 쿼리문을 실행합니다.\n" + sql);
		PreparedStatement pstmt = conn.prepareStatement(sql);
...
*/
	...
	public int getDataCount(String searchKey, String searchValue) throws SQLException
	{
		...
		String sql = "SELECT COUNT(FAQID) AS MAXPAGE FROM SRFAQ WHERE "  + searchKey + " LIKE ?";
		System.out.println("아래의 쿼리문을 실행합니다.\n" + sql);
		PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	{		
		 ...
		 * String sql = "SELECT * " +
					"FROM " +
					"(SELECT ROW_NUMBER() OVER (ORDER BY FAQID DESC) AS RNUM, DATA.* " +
					"FROM (SELECT FAQID, SRAID, FAQNAME, FAQDAP FROM SRFAQ " +
					"WHERE FAQNAME LIKE ? " +
					"ORDER BY FAQID DESC) DATA) WHERE RNUM >= ? AND RNUM <= ?";
					*/
		
		String sql = "SELECT * " +
					"FROM " +
					"(SELECT ROW_NUMBER() OVER (ORDER BY FAQID) AS RNUM, DATA.* " +
					"FROM (SELECT FAQID, SRAID, FAQNAME, FAQDAP FROM SRFAQ " +
	...
	public int maxCode() throws SQLException
	{
		...
		String sql = "SELECT MAX(FAQID) AS MAX FROM SRFAQ";
		System.out.println("maxCode 쿼리문을 실행합니다.\n" + sql);
	...
	public int insert(int cd, String title, String comment) throws SQLException
	{
		...
		String sql = "INSERT INTO SRFAQ(FAQID, SRAID, FAQNAME, FAQDAP) " +
					 "VALUES(?, 'admin', ?, ?)";
		System.out.println("insert 쿼리문을 실행합니다.\n" + sql);
		PreparedStatement pstmt = conn.prepareStatement(sql);
		...
		System.out.println(cd + " " + title + " " + comment);
	...
	public FAQDTO updateList(int cd) throws SQLException
	{
		...
		String sql = "SELECT FAQID, SRAID, FAQNAME, FAQDAP FROM SRFAQ WHERE FAQID = ?";
		System.out.println("updateList 쿼리문을 실행합니다.\n" + sql);
		...
		while(rs.next())
		{
			...
			dto.setSraid(rs.getString("SRAID"));
			dto.setFaqname(rs.getString("FAQNAME"));
			dto.setFaqdap(rs.getString("FAQDAP").replaceAll("<br>", "\n"));
		}
		rs.close();
		pstmt.close();
		conn.close();
	...
	public int update(int cd, String title, String comment) throws SQLException
	{
		...
		String sql = "UPDATE SRFAQ SET FAQNAME = ?, FAQDAP = ? WHERE FAQID = ?";
		System.out.println("아래의 쿼리문을 실행합니다.\n" + sql);
		PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	public int delete(String cd) throws SQLException
	{
		...
		String sql = "DELETE FROM SRFAQ WHERE FAQID = ?";
		System.out.println("아래의 쿼리문을 실행합니다.\n" + sql);
		PreparedStatement pstmt = conn.prepareStatement(sql);
